...
cs_ TAN_FUNC_STR[] = {
    "Тангенс. Производная: 1/cos²(x). Или sec²(x), если вы из тех, кто знает, что такое секанс."
  , "tan → 1/cos². Тригонометрия усложняется. Держитесь."
  , "Производная тангенса. Гуглится чаще, чем хотелось бы признавать."
};

cs_ CTG_FUNC_STR[] = {
    "Котангенс → -1/sin²(x). Минус на месте, всё по плану."
  , "ctg → -csc². Если вы помните, что такое косеканс — респект."
  , "Производная котангенса. Редкий зверь, но иногда встречается."
};

cs_ ASIN_FUNC_STR[] = {
    "Обратный синус. Производная с корнем в знаменателе. Готовьтесь."
  , "arcsin → 1/√(1-x²). Выглядит страшно, работает стабильно."
...
cs_ ACOS_FUNC_STR[] = {
  ...
  , "arccos → -1/√(1-x²). Отличается от арксинуса только минусом. Внимательнее."
  , "Производная арккосинуса. Спойлер: почти как у арксинуса, но не совсем."
};

cs_ ATAN_FUNC_STR[] = {
    "arctg → 1/(1+x²). Одна из самых приятных обратных тригонометрических функций."
  , "Арктангенс. Производная без корней — уже праздник."
  , "1/(1+x²). Простота, которую мы заслужили после арксинуса."
};

cs_ ACTG_FUNC_STR[] = {
    "arcctg → -1/(1+x²). Как арктангенс, но с экзистенциальным минусом."
  , "Арккотангенс. Встречается редко, но метко."
  , "Минус от арктангенса. Паттерн прослеживается, да?"
};

cs_ SQRT_FUNC_STR[] = {
    "√x — это x^(1/2). А значит, производная: 1/(2√x). Степени наше всё."
  , "Корень извлекли, теперь дифференцируем. 1/(2√x) — и готово."
...
cs_ TANH_FUNC_STR[] = {
    "th(x) → 1/ch²(x). Если вы работаете с нейросетями — эта функция вам знакома."
  , "tanh → sech². Активация RNN одобряет."
  , "Гиперболический тангенс. Любимец deep learning и производных."
};

cs_ CTH_FUNC_STR[] = {
    "cth(x) → -1/sh²(x). Минус вернулся. Скучали?"
  , "Гиперболический котангенс. Редкий гость, но формулу знать полезно."
  , "cth → -csch². Если вы дошли до этой функции — вы точно не сдаётесь."
};
